...
Various Machine Learning models to detect fraudulent use of Credit cards. Each model performance and results. The best performance is achieved using SMOTE technique.
...
With this approach, the model is able to detect almost 100% of all fraudulent transactions in the unseen test set. This fully satisfies the primary objective to detect the vast majority of abnormal transactions. In addition, the number of false positive remains acceptable and is the least amonsgt others. This means a lot less verification work (on legitimate transactions) for the fraud department. Please note that the technique and model used are simple to implement simple, easy to use and can be updated in real-time.


Comparison of key performance indicators between the tested approaches can be seen in Results.png
